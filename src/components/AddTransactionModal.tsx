...
export default function AddTransactionModal({ categories, currency = 'BHD', onClose, onSave }: Props) {
  ...
  const relevantGroups = tree.filter(g =>
    type === 'credit' ? ['INCOME', 'FINANCIAL'].includes(g.group.name) : g.group.name !== 'INCOME'
  )

  const handleKey = (k: string) => {
    setError('')
    if (k === '⌫') { setAmountExpr(a => a.slice(0, -1)); return }
    if (k === '.') { const last = amountExpr.split('+').pop() ?? ''; if (!last.includes('.')) setAmountExpr(a => a + '.'); return }
    if (k === '+') { if (amountExpr && !amountExpr.endsWith('+')) setAmountExpr(a => a + '+'); return }
    setAmountExpr(a => a + k)
  }
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-[#12101a]" style={{ fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="flex items-center px-5 py-3.5 border-b border-slate-800">
          ...
          <input
            className="flex-1 bg-transparent text-slate-200 text-right text-sm focus:outline-none placeholder-slate-600"
        ...
        <div className="flex items-center px-5 py-3.5 border-b border-slate-800">
          ...
          <input
            className="flex-1 bg-transparent text-slate-200 text-right text-sm focus:outline-none placeholder-slate-600"
            placeholder="Name of shop, brand or person"
            value={merchant}
        ...
        <div className="flex items-center px-5 py-3.5 border-b border-slate-800">
          ...
          <div className="flex-1 flex justify-end">
            <select
              className="bg-transparent text-slate-200 text-sm text-right focus:outline-none appearance-none cursor-pointer"
              value={categoryId}
      ...
      <div className="bg-[#1a1727] border-t border-slate-800">
        ...
        <div className="px-5 pt-3 pb-2">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="flex-1 text-3xl font-light text-slate-100 tracking-wider min-h-[40px]">
              {amountExpr || <span className="text-slate-600">0</span>}
            </span>
            <button
              onClick={() => setAmountExpr(a => a.slice(0, -1))}
        ...
        <div className="grid grid-cols-3">
          {NUMPAD.map(k => (
            <button
              key={k}
              onClick={() => handleKey(k)}
              className="py-5 text-2xl font-light text-slate-200 hover:bg-slate-700/60 active:bg-indigo-900/40 transition-colors border-t border-slate-700/40 select-none"
            >
              {k === '+' ? <span className="text-indigo-400 text-2xl">+</span> : k}
            </button>
          ))}
        </div>
