...
export default function Layout() {
  ...
  return (
    <div className="flex flex-col min-h-screen max-w-2xl mx-auto">
      {/* Top bar — Android-style purple header */}
      <header className="sticky top-0 z-10 bg-[#6941C6] px-4 py-3 flex items-center gap-2">
        ...
        <button className="text-white/80 text-xl px-1">🔔</button>
        <button className="text-white/80 text-xl px-1" onClick={() => navigate('/transactions')}>🔍</button>
      </header>

      {/* Page content */}
      <main className="flex-1 overflow-auto pb-20 bg-[#1a1727]">
        <Outlet />
      </main>

      {/* Bottom nav */}
      <nav className="fixed bottom-0 left-0 right-0 z-10 bg-[#1a1727] border-t border-slate-800">
